...
public class firstGoTrans {
	...
	public static void printMethod(MethodStructure3 mn, ClassStructure3 cs, boolean clinit, boolean init, boolean main) throws NotSupportedException{
		...
		if(init){
			...
			indent = "";
			System.out.print("inline " + zeroString(mn.getClassName()) + "Init(" + zeroString(mn.getClassName()) + "Type");
			indent = "    ";
			for(int i = 1; i <= numParams; i++){
			...
			if(numParams > 0){
				if(isStatic){
					...
					System.out.print("inline " + zeroString(mn.getMethodName()) + "("+ "sourceObject");
					for(int i = 1; i <= numParams; i++){
						System.out.print(", " + zeroString(localVariableNodes.get(i).name));
						createdVars.add(i);
						orderedMethodParamTypes.add(getAndAdjustSourceTypes(localVariableNodes.get(i).desc));
					...
					if(clinit){
						...
						case Constants.OPCODE_PUTFIELD:
							...
							if(unAccountedForInstanceFields.contains(((FieldInsnNode) presentNode).name)){
								...
								System.out.println(indent + zeroString(localVariableNodes.get(0).name) + "." + zeroString(((FieldInsnNode) presentNode).name) + " = " + fi7 + ";");
							}
							break;
						}
					} else {
		...
		if(init){
			for(int i = 1; i <= numParams; i++){
				System.out.println(indent + zeroString(mn.getClassName()) + "Type." + zeroString(originalLocalVariableNodeNames.get(i)) + " = " + zeroString(localVariableNodes.get(i).name) + ";");
			}
			System.out.println("}");
		} else if(clinit){
			for(FieldInsnNode s: unAccountedForStaticFields){
