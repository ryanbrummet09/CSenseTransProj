...
public class ClassReader4 {
	...
	public static void main(String args[]) throws Exception{
		SourceStructure3 test1 = new SourceStructure3(new File("/Users/ryanbrummet/Documents/cSenseProj/ASMTest/version3/Test2.class"));
		...
		for(int c = 0; c < test1.getClassStructures().size(); c++){
			...
			for(int m = 0; m < csMethodStructures.size(); m++){
				...
				if(loopStartAndStops.size() > 0){
					for(int lss = 0; lss < loopStartAndStops.size(); lss = lss + 3){
						switch(loopStartAndStops.get(lss + 2)){
						case Constants.SCOPE_FOR_EACH_LOOP:
							loopType = "SCOPE_FOR EACH LOOP";
							break;
						case Constants.SCOPE_DO_WHILE_LOOP:
							loopType = "SCOPE_DO WHILE LOOP";
							break;
						case Constants.SCOPE_FIRST_RUN_CONDITIONAL_LOOP:
							loopType = "SCOPE_FIRST_RUN_CONDITIONAL_LOOP";
							break;
						}
				...
				tempInc = 0;
				ArrayList<Integer> dudBlockIndexes = csMethodStructures.get(m).getDudBlockIndexes();
				if(dudBlockIndexes.size() > 0){
					for(int i = 0; i < dudBlockIndexes.size(); i++){
						System.out.println("        DUD " + tempInc + ": " + dudBlockIndexes.get(i));
						tempInc++;
					}
				}
				tempInc = 0;
				ArrayList<Integer> tryCatchBlockIndexes = csMethodStructures.get(m).getTryCatchBlockIndexes();
				if(tryCatchBlockIndexes.size() > 0){
					for(int i = 0; i < tryCatchBlockIndexes.size(); i = i + size + 1){
						size = tryCatchBlockIndexes.get(i);
						System.out.print("   TRYCATCH " + tempInc + ", SIZE " + tryCatchBlockIndexes.get(i) + ": ");
						int index = i + 1;
						while(index <= i + size){
							System.out.print(tryCatchBlockIndexes.get(index) + " ");
							index++;
						}
						System.out.println();
						tempInc++;
				...
				tempInc = 0;
				ArrayList<Integer> ifClassStmntBlockIndexes = csMethodStructures.get(m).getIfClassStmntBlockIndexes();
				if(ifClassStmntBlockIndexes.size() > 0){
					for(int i = 0; i < ifClassStmntBlockIndexes.size(); i = i + size + 1){
						size = ifClassStmntBlockIndexes.get(i);
						System.out.print("    IFCLASS " + tempInc + ", SIZE " + ifClassStmntBlockIndexes.get(i) + ": ");
						int index = i + 1;
						while(index <= i + size){
							System.out.print(ifClassStmntBlockIndexes.get(index) + " ");
							index++;
						}
						System.out.println();
						tempInc++;
