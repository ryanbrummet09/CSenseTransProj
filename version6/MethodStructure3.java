...
public class MethodStructure3 {
	...
	public MethodStructure3(MethodNode mn, String owner){
		...
		arrayVariableKeys = new ArrayList<VariableKey>();
	...
	/**
	 * 
	 * @return returns a true if this method is static, false otherwise
	 */
	public boolean isStatic(){
		if(!getMethodName().equals("<clinit>") && !getMethodName().equals("<init>") && getLocalVariables().size() > 0){
			if(Constants.zeroString((getLocalVariables().get(0).desc).substring(1)).equals(Constants.zeroString(getClassName()))){
				return false;
			} else {
				return true;
			}
	...
	/**
	 * 
	 * @return returns an ArrayList<Integer> where each trycatch is described by an indeterminant number of elements.
	 * Try catches given in asm are combined since some try catches are in fact
	 ...
	 * the start of the handler's code.  Each subsequent element after this is the start of a catch section with the exception of the next to
	 * last element which is the start of the handler for the finally section and the last element which is the end of the finally section.
	 */
	...
	/**
	 * 
	 * @return returns the sorted scopeIntervals of the method associated with this MethodStructure
	 */
	public ArrayList<ScopeInterval> getSortedScopeIntervals(){
		...
		for(int i = 0; i < blockStructures.size(); i++){
			...
			for(Integer temp: loopsStartingAtI){
				...
				if(temp >= 0){
						...
						for(int t = 0; t < endBlock.getNumBlockInsn(); t++){
							if(endBlock.getBlockInsnNumber(t).getType() == Constants.ABSTRACT_METHOD_INSN){
								...
								if(potMethNode.owner.equals("java/util/Iterator") && potMethNode.name.equals("hasNext") && potMethNode.getOpcode() == Constants.OPCODE_INVOKE_INTERFACE){
									tempStart.add(Constants.SCOPE_FOR_EACH_LOOP);
									break loopDefined;
								}
							}
						}
					...
					if(isDoWhileLoop){
						tempStart.add(Constants.SCOPE_DO_WHILE_LOOP);
						break loopDefined;
					}	
					...
					tempStart.add(Constants.SCOPE_FIRST_RUN_CONDITIONAL_LOOP);
				}
			}
		}	
		...
		while(0 < inBuiltTryCatchList.size()){
			...
			for(int j = 1; j < inBuiltTryCatchList.size(); j++){
				if(inBuiltTryCatchList.get(0).start == inBuiltTryCatchList.get(j).start && inBuiltTryCatchList.get(0).end == inBuiltTryCatchList.get(j).end){
					for(int i = labelToBlock(inBuiltTryCatchList.get(j).end).getMethodBlockNumber(); i < labelToBlock(inBuiltTryCatchList.get(j).handler).getMethodBlockNumber(); i++){
						for(int t = 0; t < blockStructures.get(i).getNumBlockInsn(); t++){
							if(blockStructures.get(i).getBlockInsnNumber(t).getOpcode() == Constants.OPCODE_GOTO){
								if(!breakBlockIndexes.contains(i) && !dudBlockIndexes.contains(i)){
									dudBlockIndexes.add(i);
								}
							}
						}
					}
			...
			for(int j = 1; j < inBuiltTryCatchList.size(); j++){
				if(inBuiltTryCatchList.get(0).start == inBuiltTryCatchList.get(j).start  && inBuiltTryCatchList.get(j).type == null && labelToBlock(inBuiltTryCatchList.get(j).end).getMethodBlockNumber() > (labelToBlock(inBuiltTryCatchList.get(0).handler)).getMethodBlockNumber()){
					...
					for(int i = labelToBlock(finallyBlock.end).getMethodBlockNumber(); i <  blockStructures.size(); i++){
						for(int t = 0; t < blockStructures.get(i).getNumBlockInsn(); t++){
							if(blockStructures.get(i).getBlockInsnNumber(t).getOpcode() == Constants.OPCODE_ATHROW){
								break outerLoop0;
							} else if (blockStructures.get(i).getBlockInsnNumber(t).getOpcode() == Constants.OPCODE_GOTO){
								if(!breakBlockIndexes.contains(i) && !dudBlockIndexes.contains(i)){
									dudBlockIndexes.add(i);
								}
							}
			...
			if(finallyFound){
				for(int j = 1; j < inBuiltTryCatchList.size(); j++){
					if((labelToBlock(finallyBlock.handler)).getMethodBlockNumber() == (labelToBlock(inBuiltTryCatchList.get(j).handler)).getMethodBlockNumber() && (labelToBlock(finallyBlock.start)).getMethodBlockNumber() < (labelToBlock(inBuiltTryCatchList.get(j).start)).getMethodBlockNumber()){
						outerLoop1:
						for(int i = labelToBlock(finallyBlock.end).getMethodBlockNumber(); i <  blockStructures.size(); i++){
							for(int t = 0; t < blockStructures.get(i).getNumBlockInsn(); t++){
								if(blockStructures.get(i).getBlockInsnNumber(t).getOpcode() == Constants.OPCODE_ATHROW){
			...
			if(finallyBlock != null){
				tryCatchBlockIndexes.add(6 + multiCatchList.size());
				...
				tryCatchBlockIndexes.add((labelToBlock(finallyBlock.handler)).getMethodBlockNumber());  
				outerLoop2:
				for(int i = labelToBlock(finallyBlock.end).getMethodBlockNumber(); i <  blockStructures.size(); i++){
					for(int t = 0; t < blockStructures.get(i).getNumBlockInsn(); t++){
						if(blockStructures.get(i).getBlockInsnNumber(t).getOpcode() == Constants.OPCODE_ATHROW){
							tryCatchBlockIndexes.add(i + 1);
							break outerLoop2;
						}
					}
				}
				...
				try {
					throw new NotSupportedException("All try Catches must have a finally clause");
			...
			for(int j = 0; j < loopBlockIntervalsAndTypes.get(i); j++){
				for(int a = 0; a < blockStructures.get(j).getNumBlockInsn(); a++){
					if(blockStructures.get(j).getBlockInsnNumber(a).getType() == Constants.ABSTRACT_JUMP_INSN){
						...
						if(temp > loopBlockIntervalsAndTypes.get(i) && temp <= loopBlockIntervalsAndTypes.get(i + 1) && dudBlockIndexes.contains(j) == false){
	...
	/**
	 * Gets the ScopeIntervals of this method and sorts them
	 */
	private void orderMethodScopeIntervals(){
		ScopeObject scopeObject;
		if(loopBlockIntervalsAndTypes.size() > 0){
			for(int i = 0; i < loopBlockIntervalsAndTypes.size() - 3; i = i + 3){
				for(Integer qi: new ArrayList<Integer>(loopBlockIntervalsAndTypes.subList(i, i + 3))){
					System.out.println(qi);
				}
				scopeObject = new ScopeObject(new ArrayList<Integer>(loopBlockIntervalsAndTypes.subList(i, i + 3)), loopBlockIntervalsAndTypes.get(i) + 1);
				scopeIntervals.addAll(scopeObject.getScopeIntervals());
			}
		}
		int index = 0;
		if(switchAndCasesBlockIndexes.size() > 0){
			for(int i = 0; i < switchAndCasesBlockIndexes.size(); i = index){
				scopeObject = new ScopeObject(new ArrayList<Integer>(switchAndCasesBlockIndexes.subList(i, i + switchAndCasesBlockIndexes.get(i) + 1)), Constants.SCOPE_SWITCH);
				scopeIntervals.addAll(scopeObject.getScopeIntervals());
				index = index + switchAndCasesBlockIndexes.get(i) + 1;
			}
		}
		if(tryCatchBlockIndexes.size() > 0){
			for(int i = 0; i < tryCatchBlockIndexes.size(); i = index){
				scopeObject = new ScopeObject(new ArrayList<Integer>(tryCatchBlockIndexes.subList(i, tryCatchBlockIndexes.get(i) + 1)), Constants.SCOPE_TRY_CATCH);
				scopeIntervals.addAll(scopeObject.getScopeIntervals());
				index = index + tryCatchBlockIndexes.get(i) + 1;
			}
		}
		if(ifClassStmntBlockIndexes.size() > 0){
			for(int i = 0; i < ifClassStmntBlockIndexes.size(); i = index){
				scopeObject = new ScopeObject(new ArrayList<Integer>(ifClassStmntBlockIndexes.subList(i, ifClassStmntBlockIndexes.get(i) + 1)), Constants.SCOPE_IF_CLASS);
				scopeIntervals.addAll(scopeObject.getScopeIntervals());
				index = index + ifClassStmntBlockIndexes.get(i) + 1;
			}
		}
		ArrayList<ScopeInterval> tempSortedInterval = new ArrayList<ScopeInterval>();
		ScopeInterval tempScopeInterval;
		int tempIndex;
		for(int i = 0; i < scopeIntervals.size(); i = 0){
			tempScopeInterval = scopeIntervals.get(0);
			tempIndex = 0;
			for(int j = 1; j < scopeIntervals.size(); j++){
				if(tempScopeInterval.getIntervalStart() > scopeIntervals.get(j).getIntervalStart()){
					tempScopeInterval = scopeIntervals.get(j);
					tempIndex = j;
				}
			}
			tempSortedInterval.add(scopeIntervals.remove(tempIndex));
		}
		scopeIntervals = tempSortedInterval;
	}
	...
	private ArrayList<VariableKey> arrayVariableKeys;
